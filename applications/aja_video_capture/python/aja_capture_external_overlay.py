...
class CircleImageOp(hs.core.Operator):
    ...
    def _validate_circle_config(self):
        ...
        if not 0 <= self.center_x < self.width:
            raise ValueError(f"center_x must be in [0, {self.width}), got {self.center_x}")
        if not 0 <= self.center_y < self.height:
            raise ValueError(f"center_y must be in [0, {self.height}), got {self.center_y}")
...
class AJACaptureApp(Application):
    ...
    def compose(self):
        ...
        holoviz_to_videobuffer = HolovizOp(
            self, name="holoviz_to_videobuffer", **self.kwargs("holoviz_to_videobuffer")
        )
        holoviz_aja_consumer = HolovizOp(
            self, name="holoviz_aja_consumer", **self.kwargs("holoviz_aja_consumer")
        )
        ...
        self.add_flow(
            holoviz_to_videobuffer, source, {("render_buffer_output", "overlay_buffer_input")}
        )
